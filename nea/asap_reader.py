...
def tokenize(string):
	...
	for index, token in enumerate(tokens):
		#nltk.word_tokenize will seperate "@word" into ["@","word"]. Following code will 
		#reserve it back to "@word" and remove the digits from "word"
...
def read_dataset(file_path, prompt_id, maxlen, vocab, tokenize_text, to_lower, score_index=6, char_level=False):
	...
	with codecs.open(file_path, mode='r', encoding='UTF8') as input_file:
		...
		for line in input_file:
			...
			if essay_set == prompt_id or prompt_id <= 0: #filter the expected prompt
				if to_lower:
					content = content.lower()
				if char_level:
				...
				if char_level:
					...
					for word in content: #an indice is the array of content tokens
						if is_number(word):
							...
							unk_hit += 1
						total += 1
				data_x.append(indices) 			#datax is the array of all contents
				data_y.append(score)			#datay is the array of scores
				prompt_ids.append(essay_set)	#prompt_ids is the array of essay_se
				if maxlen_x < len(indices):
					maxlen_x = len(indices)		#maxlen is the total number of essays
